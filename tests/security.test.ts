jest.mock('@prisma/client', () => ({
  PrismaClient: class {
    camera = { findMany: jest.fn(), findUnique: jest.fn() };
...
describe('Security Measures', () => {
  describe('CSRF Protection', () => {
    it('should allow GET requests without CSRF header', async () => {
      const res = await request(app).get('/api/v1/auth/login'); // Or any GET
    ...
    it('should reject POST requests without CSRF header', async () => {
      const res = await request(app).post('/api/v1/auth/login').send({ email: 'test@example.com', password: '123' });
