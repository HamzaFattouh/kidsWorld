...
import { Calendar, Users, ShieldCheck, CheckCircle2, Clock, ArrowRight, UserCheck } from 'lucide-react';
...
export function AttendancePage() {
  ...
  // Role simulation toggle: 'admin' (shows all classes & teacher calendar) | 'teacher' (shows only assigned class)
  const isTeacherRole = user?.role === 'TEACHER';
  const [viewRole, setViewRole] = useState(isTeacherRole ? 'teacher' : 'admin');
  ...
  // All classes
  const allClasses = [
    { id: 'class-birds-3-4', name: 'روضة العصافير 🐥', teacherEmail: 'nora@kidsworld.com', teacherName: 'أ. نورة النابلسي' },
    { id: 'class-flowers-4-5', name: 'روضة الزهور 🌸', teacherEmail: 'sara@kidsworld.com', teacherName: 'أ. سارة الخالد' },
    { id: 'class-hope-2-3', name: 'روضة الأمل 🌟', teacherEmail: 'mona@kidsworld.com', teacherName: 'أ. منى التميمي' },
  ];

  // Filter classes if in Teacher mode: Show ONLY assigned class for teacher!
  const teacherAssignedClass = allClasses[0]; // e.g., 'روضة العصافير' assigned to teacher
  const visibleClasses = viewRole === 'teacher' ? [teacherAssignedClass] : allClasses;

  // Initial student attendance data STRUCTURED BY DAY (selectedDay -> classId -> students)
    ...
    for (let day = 1; day <= 30; day++) {
      initial[day] = {
        ...
        'class-flowers-4-5': [
          ...
          { id: 'c5', name: 'سلمى إبراهيم حامد', parent: 'إبراهيم حامد', present: day % 4 !== 0 },
        ],
        'class-hope-2-3': [
  ...
  // Initial teacher attendance data STRUCTURED BY DAY (selectedDay -> teachers)
  const [teacherAttendanceByDay, setTeacherAttendanceByDay] = useState(() => {
    ...
    for (let day = 1; day <= 30; day++) {
      initial[day] = [
        ...
        { id: 't2', name: 'أ. سارة الخالد', class: 'روضة الزهور', time: '07:45 ص', present: day % 5 !== 0 },
        { id: 't3', name: 'أ. منى التميمي', class: 'روضة الأمل', time: '08:00 ص', present: day % 3 !== 0 },
      ];
    }
    return initial;
  });
  ...
  const handleDateSquareClick = (dayNum) => {
    ...
    if (calendarMode === 'students') {
      if (viewRole === 'teacher') {
        // If teacher, skip class selection and go straight to assigned class checklist!
  ...
  // Toggle student attendance for the specific selectedDay & classId
  ...
  // Toggle teacher attendance for the specific selectedDay
  ...
  // Get active student checklist for current selectedDay and selectedClass
  ...
  // Get active teacher checklist for current selectedDay
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <PageHeader
        ...
        description="اختر مربع التاريخ في التقويم لعرض حضور وغياب اليوم المحدد"
      />

      {/* Role View Toggle (Admin vs Teacher mode) */}
      <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center text-xs font-bold text-amber-900">
          <UserCheck className="w-5 h-5 ml-2 text-amber-600" />
          <span>وضع المعاينة الحالي:</span>
          <span className="mr-2 bg-amber-200 px-2.5 py-1 rounded-lg text-amber-900">
            {viewRole === 'admin' ? 'مدير النظام (عرض كل الصفوف وتقويم المعلمين)' : 'المعلمة (عرض الصف المسؤول عنها فقط)'}
          </span>
        </div>

        <button
          onClick={() => {
            const next = viewRole === 'admin' ? 'teacher' : 'admin';
            setViewRole(next);
        ...
        >
          التبديل إلى {viewRole === 'admin' ? 'عرض المعلمة' : 'عرض الأدمن'}
        </button>
      </div>
      ...
      <div className="flex bg-gray-100 p-1.5 rounded-2xl max-w-xl">
        ...
        {/* Teacher Calendar is visible ONLY in Admin Mode! */}
        {viewRole === 'admin' && (
          <button
            onClick={() => {
          ...
          >
            ...
            تقويم المعلمين 👩‍🏫 (خاص بالأدمن فقط)
          </button>
        )}
      </div>
      ...
      {calendarMode === 'students' && (
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-7 gap-3">
                ...
                {calendarDays.map((d) => (
                  ...
                  >
                    ...
                    >
                      {viewRole === 'teacher' ? 'عرض حضور صفك' : 'عرض الصفوف'}
                    </span>
                  </div>
                ))}
              </div>
          ...
          {studentStep === 'classes_list' && viewRole === 'admin' && (
          ...
          {/* STEP 3: Students Checklist with FULLY FUNCTIONAL Checkboxes per Date */}
          {studentStep === 'students_checklist' && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-6">
              <div className="flex justify-between items-center border-b border-gray-100 pb-4">
                <button
                  onClick={() => (viewRole === 'teacher' ? setStudentStep('date_grid') : setStudentStep('classes_list'))}
                  className="flex items-center text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-xl hover:bg-blue-100"
                >
                  <ArrowRight className="w-4 h-4 ml-1" />
                  {viewRole === 'teacher' ? 'الرجوع للتقويم الشهري' : 'الرجوع لقائمة الصفوف'}
                </button>
      ...
      {calendarMode === 'teachers' && viewRole === 'admin' && (
        <div className="space-y-6">
                ...
                {activeTeacherList.map((t) => (
                  ...
                  >
                    <div
                      ...
                      onClick={() => toggleTeacherCheck(t.id)}
                    >
                      <input
                        type="checkbox"
                        checked={t.present}
