  ...
  return (
    <div className="min-h-screen bg-background dark:bg-background-dark flex flex-col font-sans overflow-x-hidden">
      ...
      >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex-shrink-0 flex items-center">
              <Link to="/" className="flex items-center gap-3 group">
                <div className="relative w-12 h-12 flex items-center justify-center bg-white rounded-full p-1 shadow-md group-hover:scale-105 transition-transform">
                  <img src="/images/logo.jpg" alt="عالم الأطفال" className="w-full h-full object-contain rounded-full" />
                </div>
                <div className="flex flex-col">
                  <span className={`text-2xl font-display font-bold leading-none ${scrolled ? 'text-brand-dark dark:text-white' : 'text-white'}`}>
                    عالم الأطفال
                  </span>
                  <span className={`text-[0.65rem] font-bold tracking-[0.2em] uppercase ${scrolled ? 'text-brand-dark/70 dark:text-gray-400' : 'text-white/80'}`}>
                    تعلم والعب
                  </span>
                </div>
              </Link>
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map(link => (
                <button 
                  ...
                  className={`font-display font-semibold transition-colors hover:text-brand-yellow ${
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              {isAuthenticated ? (
                <Link to={user?.role === 'ADMIN' ? '/admin' : user?.role === 'TEACHER' ? '/teacher' : '/parent'} className="ms-4">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-gray-800 shadow-xl absolute w-full top-full start-0">
